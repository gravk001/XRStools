...
def makepzprofile(element,filename='./xrstools/things/ComptonProfiles.dat'):
	...
	for n in range(len(binden)):
		intf             = interpolate.splrep(roughtheory[:,0],roughtheory[:,n+2],s=0.000000001,k=3) # skip the column with the total J for now
...
def findgroups(scans):
	...
	allscannames.sort() #= sorted(allscannames,key=lambda x: x.gettype)
	allscans = []
...
def appendscans(groups):
	...
	if 'long' in grouptypes:
		...
		for n in range(len(allgroups[1:])):
			if allgroups[n+1].getestart()-allgroups[n].geteend() > 4.0*allgroups[n+1].getmeanegridspacing(): # worry only if scan groups are far apart
