...
# from  pylab import  *
# ion()
# ROI_widget_roi_small.H5.xz

os.system("unxz -k ROI_widget_roi_small.H5.xz")

from XRStools import xrs_read, roifinder_and_gui, xrs_extraction, roiSelectionWidget, xrs_rois
...
# roi = w4r.getRoiObj()
roi=xrs_rois.roi_object()


roi.loadH5(os.path.join(rois_path,'ROI_widget_roi_small.H5'))
...
lw.get_spectrum_new(method='sum', include_elastic=True, interpolation=True)

print( " ELOSS ",  lw.eloss ) 
...
check_results( os.path.join(save_path,'h2o_sum_lq_small.dat') , "h2o_sum_lq_small.dat.ref"   )
...
check_results( os.path.join(save_path,'h2o_sum_mq_small.dat') , "h2o_sum_mq_small.dat.ref"   )
...
check_results( os.path.join(save_path,'h2o_sum_hq_small.dat') , "h2o_sum_hq_small.dat.ref"   )
