...
def get_spots_mask( A, median_size=None, nofroi=120,  give_borders=False, give_diff_coeffs=False, tval=-1) :

      # A[:216]=0
      # A[229:]=0
      EdfFile.EdfFile("res.edf","w+").WriteImage({},A)  
...
if __name__=="__main__":
  ...
  thematrix_=np.zeros_like(tmp)
  thematrix_[:]+=tmp
  for i in range(1,0):
    thematrix_[:,i:] +=tmp[:,:-i]
    thematrix_[:,:-i] +=tmp[:,i:]
  
  # thematrix_[:,1:] +=tmp[:,:-1]#*0.9
  # thematrix_[:,2:] +=tmp[:,:-2]#*0.8
  # thematrix_[:,3:] +=tmp[:,:-3]#*0.4
  # thematrix_[:,4:] +=tmp[:,:-4]#*0.3
  # thematrix_[:,5:] +=tmp[:,:-5]#*0.1
  # thematrix_[:,:-1]+=tmp[:,1:]#*0.9
  # thematrix_[:,:-2]+=tmp[:,2:]#*0.8
  # thematrix_[:,:-3]+=tmp[:,3:]#*0.6
  # thematrix_[:,:-4]+=tmp[:,4:]#*0.3
  # thematrix_[:,:-5]+=tmp[:,5:]#*0.1
  # thematrix_[:]=tmp

  thematrix = np.zeros(    [thematrix_.shape[0]+4, thematrix_.shape[1]+4 ]  , thematrix_.dtype)

  thematrix[2:-2   ,2:-2 ] = thematrix_
  
  thematrix=scipy.ndimage.filters.gaussian_filter(thematrix, 3.0) # order=0, output=None, mode='reflect', cval=0.0, truncate=4.0)[source]
  ...
  if (1):
      ...
      mask, diff_coeffs = get_spots_mask( submatrix  , give_diff_coeffs=True ) 
